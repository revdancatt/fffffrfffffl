...
const page = {
  ...
  doDecimate: (lines, dDistance) => {
    ...
    lines.forEach((line) => {
      ...
      for (let pi = 0; pi < points.length - 1; pi++) {
        ...
        const times = parseInt(distance / dDistance)
  ...
  wrapSVG: async (lines, size, filename, scale) => {
    ...
    lines.forEach(line => {
      ...
      output += `M ${points[0].x * size[0]} ${points[0].y * size[0]} `
      for (let p = 1; p < points.length; p++) {
        output += `L ${points[p].x * size[0]} ${points[p].y * size[0]} `
      }
    })
