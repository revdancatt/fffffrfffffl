...
const ratio = 9 / 16 // 1.414 is the golden ratio
...
const makeFeatures = () => {
  ...
  if (ratio > 1) {
    ...
    features.topBottomBorder = 0.075
    features.squaresDown = possibleSquaresAcross[Math.floor(fxrand() * possibleSquaresAcross.length)]
    // Work out the size of the squares
    features.squareSize = (1 - (features.topBottomBorder * 2)) / features.squaresDown
    // Work out the number of squares down the page
    features.squaresAcross = Math.floor(((1 / ratio) - (features.topBottomBorder * 2)) / features.squareSize)
    features.sideBorder = ((1 / ratio) - (features.squaresAcross * features.squareSize)) / 2
  }
  ...
  if (!features.linesSet) {
    for (let layer = 0; layer < features.layers; layer++) {
      ...
      for (let i = 0; i < features.squares[layer].length; i++) {
        ...
        const outterCorners = {
          tl: {
            x: -square.size / 2 * wMod - square.size * wMod,
            y: -square.size / 2 * wMod - square.size * wMod
          },
          tr: {
            x: square.size / 2 * wMod + square.size * wMod,
            y: -square.size / 2 * wMod - square.size * wMod
        ...
        ctx.save()
        ctx.translate(square.middle.x * wMod, square.middle.y * wMod)

        // This is the polygon that we're going to use to cull the lines
        const cullingPoly = [
          [-square.size / 2 * wMod, -square.size / 2 * wMod],
          [square.size / 2 * wMod, -square.size / 2 * wMod],
          [square.size / 2 * wMod, square.size / 2 * wMod],
          [-square.size / 2 * wMod, square.size / 2 * wMod]
        ]
        ...
        for (let j = 0; j < lines; j++) {
          ...
          if (newLine) {
            const screenLine = {
              p1: getTransformToScreen(newLine.p1.x, newLine.p1.y, square.rotate * Math.PI / 180, square.middle.x * wMod, square.middle.y * hMod),
              p2: getTransformToScreen(newLine.p2.x, newLine.p2.y, square.rotate * Math.PI / 180, square.middle.x * wMod, square.middle.y * hMod)
            }
    ...
    for (let layer = 0; layer < features.layers; layer++) {
      ...
      for (let i = 0; i < features.lineHolder[layer].lines.length; i++) {
        ...
        const thisLine = []
        if (line.p1.y < (wMod - (features.sideBorder * wMod) || line.p2.y < wMod - (features.sideBorder * wMod))) {
          thisLine.push({
            x: (line.p1.y - (features.topBottomBorder * wMod) + (features.sideBorder * wMod)) / wMod,
            y: line.p1.x / wMod
          })
          thisLine.push({
            x: (line.p2.y - (features.topBottomBorder * wMod) + (features.sideBorder * wMod)) / wMod,
            y: line.p2.x / wMod
          })
        } else {
          thisLine.push({
    ...
    for (let y = 0; y < features.squaresDown; y++) {
      ...
      for (let x = 0; x < features.squaresAcross; x++) {
        ...
        const square = {
          top: (y * features.squareSize * wMod) + (features.topBottomBorder * wMod),
          bottom: ((y + 1) * features.squareSize * wMod) + (features.topBottomBorder * wMod),
          left: (x * features.squareSize * wMod) + (features.sideBorder * wMod),
          right: ((x + 1) * features.squareSize * wMod) + (features.sideBorder * wMod)
        }
        square.center = {
