/* global preloadImagesTmr fxhash noise */
  ...
  while (mapFilledPercentage > 40 && mapEscape < 100) {
    ...
    while (rooms.length < roomCount && escapeCounter < 100) {
      ...
      for (let i = 0; i < roomCount; i++) {
        const w = minRoomSize + Math.floor(R.prng() * (maxRoomSize - minRoomSize))
        const h = minRoomSize + Math.floor(R.prng() * (maxRoomSize - minRoomSize))
        const x = Math.floor(R.prng() * (width - w - 1)) + 1
        const y = Math.floor(R.prng() * (height - h - 1)) + 1
...
const makeFeatures = () => {
  ...
  if (R.prng() < 0.333) {
    const override = overRides[Math.floor(R.prng() * overRides.length)]
  ...
  features.palette = palettes[Math.floor(R.prng() * palettes.length)]
  features.palette = palettes[1]
  features.backgroundColour = '#333'
  features.backgroundMode = 'flat'
  const firstColour = backgrounds[Math.floor(R.prng() * backgrounds.length)]
  let secondColour = backgrounds[Math.floor(R.prng() * backgrounds.length)]
  // Make sure the second colour isn't the same as the first
  while (secondColour === firstColour) {
    secondColour = backgrounds[Math.floor(R.prng() * backgrounds.length)]
  }
  ...
  if (R.prng() < 0.666) {
    ...
    features.backgroundColour = backgrounds[Math.floor(R.prng() * backgrounds.length)]
    ...
    if (R.prng() < 0.5) {
      ...
      if (R.prng() < 0.2) features.sunGlow = true
    }
    ...
    if (R.prng() < 0.25) {
      ...
      const newBackground = features.palette.tiles.splice(Math.floor(R.prng() * features.palette.tiles.length), 1)[0]
  ...
  features.showPixels = R.prng() < 0.5

  // Make some pixels in the background
  features.pixels = {
    across: Math.floor(R.prng() * 40) + 20
  }
  features.pixels.down = Math.floor(features.pixels.across * ratio)
  const pNoiseOffset = R.prng() * 4000 + 2000
  ...
  for (let i = 0; i < features.mapSize.floorSizes.length; i++) {
    features.maps.push(generateMap({
      ...
      floorColour: features.palette.tiles[Math.floor(R.prng() * features.palette.tiles.length)]
    }))
  }
  // Now build up a long list of all the colours we'll use, about 10,000 of them
  features.colours = []
  for (let i = 0; i < 10000; i++) {
    features.colours.push(features.palette.tiles[Math.floor(R.prng() * features.palette.tiles.length)])
  }
    ...
    for (let j = 0; j < features.maps[i].rooms.length; j++) {
      ...
      for (const cornerName of cornerNames) {
        ...
        while (R.prng() < cornerDoChance) {
          ...
          const direction = validDirections[Math.floor(R.prng() * validDirections.length)]
          // const direction = 'ne'
